$(window).on('load', function() {
    ...
    $('.wishlist-remove').click(function(e){
        ...
        $.ajax({
            ...
            dataType: "json", 
            success: function (data) {
                showToast('', 'Xóa sản phẩm khỏi yêu thích thành công', "success");
                reLoad();
            },
            error: function (request, status, error) {
                showToast('', 'Không thể xóa sản phẩm khỏi yêu thích được', "error");
                reLoad();
            }
        });
    });
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == "error") {
                    ...
                    var type = "warning";
                    if (data == 'exist') {
                        content = "Sản phẩm này đã có trong yêu thích";
                    } else if (data == "fail") {
            ...
            error: function (request, status, error) {
                showToast('', 'Không thể thêm sản phẩm vào yêu thích được', "error");
                reLoad();
            }
        });
    });
    ...
    $('.btn-cart').click(function(e){
        ...
        $.ajax({
            ...
            success:function (data) {
                if(data == "error") {
                    window.location = "404.html";
                } else if(data == "full cart") {
                    showToast('', 'Vượt quá số lượng sản phẩm', 'warning');
                } else if (data.length > 0) {
                    let str = '<div class="basket"><a class="basket-icon">'+
                    '<i class="fa fa-shopping-basket"></i> GIỎ HÀNG <span class="badge">'+
                    data[1]+'</span></a>'+
