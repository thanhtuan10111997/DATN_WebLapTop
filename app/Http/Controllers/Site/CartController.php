...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request)
    {
        if ($this->checkLogIn()) {
            ...
            $data = array_filter($cart, function ($item) use ($id) {
                return $item->id == 1;
            });
            dd ($data);
            if (Cart::instance(Auth::user()->id)
                ->search(function ($cartItem) use ($id) {
                    return $cartItem->id === $id;
                })->qty + 1 > $product->quantities
            ) {
                return response()->json('full cart');
            }
    ...
    public function add(Request $request)
    {
        if ($this->checkLogIn()) {
            ...
            $data = $cart->search(function ($cartItem) use ($id) {
                    return $cartItem->id == $id;
                })
                ->first();
            if ($data && ($data->qty + $value > $product->quantities)) {
                return response()->json('full cart');
            }
            Cart::instance(Auth::user()->id)->add(
                [
