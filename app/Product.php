...
class Product extends Model
{
    ...
    public function scopeSearchMul($query, $category_id, $price, $order)
    {
        switch ($price) {
            case 1:
                $query = $query->whereRaw('(price - discount) < 8000000');
                break;

            case 2:
                $query = $query->whereRaw('(price - discount) >= 8000000 and (price - discount) <= 10000000');
                break;

            case 3:
                $query = $query->whereRaw('(price - discount) >= 10000000 and (price - discount) <= 12000000');
                break;

            case 4:
                $query = $query->whereRaw('(price - discount) > 12000000');
                break;
