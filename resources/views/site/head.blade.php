...
<head>
    ...
    <title>{{ __('Cửa hàng Thanh Tuấn') }}</title>
    <!-- Favicons Icon -->
...
<div style="font-family:Arial" id="jtv-mobile-menu">
    <ul>
                ...
                {!! Form::open(['route' => 'site.home.search', 'method' => 'get', 'id' => 'mob-search']) !!}
                    <div class="input-group">
                        <div class="input-group-btn">
                            {!! Form::text('key', null, ['class' => 'form-control simple', 'placeholder' => 'Bạn muốn tìm gì...', 'id' => 'srch-term', 'autocomplete' => 'off']) !!}
        ...
        @endforeach
        <li class="nosub">
            <a style="font-family:Arial" href="{{ route('site.news.index') }}">{{ __('tin tức') }}</a>
        </li>
        <li class="nosub">
            <a style="font-family:Arial" href="{{ route('site.news.index') }}">{{ __('giới thiệu') }}</a>
        </li>
        <li class="nosub">
            <a style="font-family:Arial" href="{{ route('site.contact.index') }}">{{ __('liên hệ') }}</a>
        </li>
    </ul>
    <div class="top-links">
        <ul class="links">
            @if(Auth::check())
                <li>
                    <a title="Tài khoản của tôi" href="{{ route('site.customer.edit', ['customer' => Auth::user()->id]) }}">{{ Auth::user()->name }}</a>
                </li>
                <li>
                    <a title="Danh sách yêu thích của tôi" href="{{ route('site.cart.view_to_wishlist') }}">Danh sách yêu thích
                        <span class="wl_sl" class="badge">{{ $wl_sl > 0 ? $wl_sl : ""}}</span></a>
                </li>
                <li><a title="Checkout" href="{{ route('site.cart.checkout') }}">Kiểm tra đặt hàng</a></li>
                <li><a title="Đăng xuất" href="{{ route('site.customer.logout') }}">Đăng xuất</a></li>
            @else
                <li><a title="Đăng ký" href="{{ route('site.customer.regester') }}">Đăng ký</a></li>
                <li><a title="Đăng nhập" href="{{ route('site.customer.login') }}">Đăng nhập</a></li>
            @endif
        </ul>
    </div>
</div>
            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-3 col-sm-3 col-xs-12">
                        <!-- Header Logo -->
                        <div class="logo"><a title="Trang chủ" href="{{ route('site.home.index') }}"><img alt="Laptop Thanh Tuấn" src="{{ asset('source/bower_components/library/site/version4/images/logo.png') }}"></a></div>
                        <!-- End Header Logo -->
                        <div class="nav-icon">
                            <div class="mega-container visible-lg visible-md visible-sm">
                                <div class="navleft-container">
                                    <div class="mega-menu-title">
                                        <h3 style="font-weight:bold;"><i class="fa fa-navicon"></i>{{ __('Danh mục') }}</h3>
                                    </div>
                                    <div class="mega-menu-category">
                                        <ul class="nav">
                                            ...
                                            <li class="nosub">
                                                <a style="font-family:Arial" href="{{ route('site.about.index') }}">{{ __('giới thiệu') }}</a>
                                            </li>
                                            <li class="nosub">
                                                <a style="font-family:Arial" href="{{ route('site.contact.index') }}">{{ __('liên hệ') }}</a>
                                            </li>
                        ...
                        <div class="jtv-header-box">
                            <div class="search_cart_block">
                                <div class="search-box hidden-xs">
                                    ...
                                    {!! Form::text('key', null, ['class' => 'searchbox search', 'placeholder' => 'Bạn muốn tìm gì...', 'id' => 'search', 'autocomplete' => 'off']) !!}
                                ...
                                <div class="right_menu">
                                    <div class="menu_top">
                                        <div class="top-cart-contain pull-right" style="margin-left: 15px">
                                            <div class="mini-cart">
                                                <div class="basket"><a class="basket-icon" href="{{ route('site.cart.view_to_compare') }}"><i class="fa fa-balance-scale"></i>SO SÁNH<span class="badge" id="so-luong">{{ $compare_sl > 0 ? $compare_sl : "" }}</span></a>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="top-cart-contain pull-right">
                                            <div class="mini-cart" id="cart-content">
                                                <div class="basket"><a class="basket-icon"><i class="fa fa-shopping-basket"></i>GIỎ HÀNG<span class="badge">{{ count($cart_init) > 0 ? count($cart_init) : "" }}</span></a>
                                                        ...
                                                        @if(count($cart_init) > 0)
                                                            ...
                                                            <ul class="mini-products-list" id="cart-sidebar">
                                                                @foreach($cart_init as $row)
                                                                    <li class="item">
                                                                        <div class="item-inner"><a class="product-image" title="" href="{{ route('product.view', ['id' => $row->id ]) }}"><img alt="product tilte is here" src="{{ url(config('app.imageUrl')) }}/{{ $row->options->avatar }}"></a>
                                ...
                                <div class="toplinks">
                                    ...
                                    <ul class="links">
                                        @if(Auth::check())
                                            ...
                                            <li>
                                                <a title="Danh sách yêu thích" href="{{ route('site.cart.view_to_wishlist') }}">♥Yêu thích
                                            ...
                                            <li>
                                                <a title="Đăng xuất" href="{{ route('site.customer.logout') }}">Đăng xuất</a>
                                            </li>
                                        @else
                                            <li><a title="Đăng ký" href="{{ route('site.customer.regester') }}">Đăng ký</a></li>
                                            <li><a title="Đăng nhập" href="{{ route('site.customer.login') }}">Đăng nhập</a></li>
